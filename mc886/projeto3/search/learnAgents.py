...
class QLearnAgent(Agent):
    ...
    def getAction(self, state, debug_mode=True):
        ...
        # construct s'
        curr_state = (str(legal), str(pacman_position),
                      str(ghost_positions), str(food_locations))
        if debug_mode:
    ...
    def update_Q_value(self, state, final_step=False):
        ...
        # update Q(s, a)
        self.Q_values[self.prev_state][self.prev_action] += (self.alpha * (
            reward + self.gamma * max_Q_value - self.Q_values[self.prev_state][self.prev_action]))
