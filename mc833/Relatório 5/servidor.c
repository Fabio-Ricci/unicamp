...
void show_connected_info(int sockfd, struct sockaddr_in addr)
{
   char name[128];
   time_t ticks;
   ...
   printf("Client with IP: %s Port: %d connected time: %s\n", name, ntohs(addr.sin_port), ctime(&ticks));
}

void show_disconnected_info(int sockfd, struct sockaddr_in addr)
{
   char name[128];
   time_t ticks;
...
void handle_client(int connfd, struct sockaddr_in addr)
{
   ...
   show_connected_info(connfd, addr);

   for (;;)
   {
      // le comando recebido do cliente a ser executado
   ...
   show_disconnected_info(connfd, addr);
